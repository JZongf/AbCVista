...
def dgram_from_positions(
    ...
    # original_shape = dgram.shape[1]
    # dgram = dgram.squeeze(-1).squeeze(0)
    # dgram_rebuilder = run_vae.DistanceRebuilder(dgram, image_size=256, device=pos.device)
    # dgram = dgram_rebuilder.rebuild(dgram)
    # dgram = dgram[..., :original_shape, :original_shape]
    # dgram = dgram.unsqueeze(-1)
