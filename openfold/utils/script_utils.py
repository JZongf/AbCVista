...
def load_models_from_command_line(config, model_device, openfold_checkpoint_path, jax_param_path, output_dir, args):
    ...
    if jax_param_path:
        for path in jax_param_path.split(","):
            ...
            model = AlphaFold(config, args=args)
            model = model.eval()
            import_jax_weights_(
    ...
    if openfold_checkpoint_path:
        for path in openfold_checkpoint_path.split(","):
            ...
            peft_config = LoraConfig(
                ...
                lora_alpha=32, 
                target_modules=[
                    "linear", "linear_q", "linear_k", "linear_v", "linear_o",
                    ],
                lora_dropout=0.1,
                inference_mode=True,
            )
    ...
    if not jax_param_path and not openfold_checkpoint_path:
        raise ValueError(
            "At least one of jax_param_path or checkpoint_path must "
            "be specified."
        )
