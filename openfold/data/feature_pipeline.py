...
def np_to_tensor_dict(
    ...
    """ 
    tensor_dict = {
        k: torch.from_numpy(np.array(v)) for k, v in np_example.items() if k in features
    }
...
def np_example_to_features(
    ...
    np_example["region_index"] = np_example.get("region_index", None)
    np_example["pair_msa_num"] = np_example.get("pair_msa_num", None)
    np_example["chain1_msa_num"] = np_example.get("chain1_msa_num", None)
    np_example["chain2_msa_num"] = np_example.get("chain2_msa_num", None)
    np_example["msa_cluster_idx"] = np_example.get("msa_cluster_idx", None)
    if np_example["msa_cluster_idx"] is not None:
        cluster = True
    else:
        cluster = False
    ...
    with torch.no_grad():
        if(not is_multimer):
            features = input_pipeline.process_tensors_from_config(
                ...
                cfg.common,
                cfg[mode],
                region_index=np_example["region_index"],
            )
        else:
            features = input_pipeline_multimer.process_tensors_from_config(
                ...
                cfg[mode],
                region_index=np_example["region_index"],
                pair_msa_num=np_example["pair_msa_num"],
                chain1_msa_num=np_example["chain1_msa_num"],
                chain2_msa_num=np_example["chain2_msa_num"],
                msa_cluster_idx=np_example["msa_cluster_idx"],
                cluster=cluster,
                msas_num=np_example["msas_num"],
            )
