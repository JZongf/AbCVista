...
ANTIBODY_REGION = ["FR1", "CDR1", "FR2", "CDR2", "FR3", "CDR3", "FR4"]
CHAIN_TO_ID = {"H":1, "L":2}
...
def make_mmcif_features(
    ...
    mmcif_feats["release_date"] = np.array(
        [mmcif_object.header.get("release_date", "2010-01-01").encode("utf-8")], dtype=np.object_
    )
...
def make_msa_features(msas: Sequence[parsers.Msa], deduplication=True) -> FeatureDict:
    ...
    for msa_index, msa in enumerate(msas):
        ...
        for sequence_index, sequence in enumerate(msa.sequences):
            if sequence in seen_sequences and deduplication:
                continue
...
def add_assembly_features(
    ...
    for idx, (chain_id, chain_features) in enumerate(all_chain_features.items()):
      grouped_chains[idx+1].append(chain_features)
...
class DataPipeline:
    ...
    def _parse_msa_data(
        ...
        if(alignment_index is not None):
            ...
            for f in sorted(os.listdir(alignment_dir)):
    ...
    def _parse_template_hit_files(
        ...
        input_sequence: str,
        alignment_index: Optional[Any] = None,
        chain_features: Optional[FeatureDict] = None,
    ) -> Mapping[str, Any]:
        all_hits = {}
        if(alignment_index is not None):
            fp = open(os.path.join(alignment_dir, alignment_index["db"]), 'rb')
...
class DataPipelineMultimer:
    ...
    def _process_single_chain(
        ...
        with temp_fasta_file(chain_fasta_str) as chain_fasta_path:
            chain_features = {}
            # Add antibody-specific features, if available
            if os.path.exists(os.path.join(chain_alignment_dir, "region_index.json")):
                with open(os.path.join(chain_alignment_dir, "region_index.json"), "r") as fp:
                    region_index = json.load(fp)
                chain_features["region_index"] = region_index
                chain_features["chaintype_id"] = np.repeat(CHAIN_TO_ID[region_index["chain_type"]], len(sequence))
                region_id = np.zeros(len(sequence))
                for i, region in enumerate(ANTIBODY_REGION):
                    _region = region_index[region]
                    region_id[_region[0]:_region[1]] = i + 1
                chain_features["region_id"] = region_id
            else:
                chain_features["region_index"] = {"chain_type":"O",
                                                  "length":len(sequence),}
                chain_features["chaintype_id"] = np.repeat(0, len(sequence))
                chain_features["region_id"] = np.zeros(len(sequence))

            chain_features["multimer"] = not is_homomer_or_monomer
            
            seq_chain_features = self._monomer_data_pipeline.process_fasta(
                fasta_path=chain_fasta_path,
                alignment_dir=chain_alignment_dir,
                chain_features=chain_features,
            )
            chain_features.update(seq_chain_features)
    ...
    def _all_seq_msa_features(self, fasta_path, alignment_dir):
        """Get MSA features for unclustered paired msa."""
        paired_msa_path = os.path.join(alignment_dir, "paired_hits.a3m")
        with open(paired_msa_path, "r") as fp:
            uniprot_msa_string = fp.read()
        msa = parsers.parse_a3m(uniprot_msa_string)
        all_seq_features = make_msa_features([msa], deduplication=False)
    ...
    def process_fasta(self,
        ...
        all_chain_features = {}
        sequence_features = {}
        is_homomer_or_monomer = len(input_seqs) == 1
    ...
    def process_mmcif(
        ...
        all_chain_features = {}
        sequence_features = {}
        is_homomer_or_monomer = len(list(mmcif.chain_to_seqres.values())) == 1
