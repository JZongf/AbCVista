...
def parse(
    *, file_id: str, mmcif_string: str, catch_all_errors: bool = True
    ...
    try:
        ...
        return ParsingResult(mmcif_object=None, errors={(file_id, ""): e})
    ...
    Full_chain = True
    if mmcif_loop_to_list("_entity_poly_seq.", parsed_info) != []:
        valid_chains = _get_protein_chains(parsed_info=parsed_info)
    else:
        valid_chains = _get_protein_chains_uncomplete(parsed_info=parsed_info)
        Full_chain = False
    if not valid_chains:
    ...
    for atom in _get_atom_site_list(parsed_info, Full_chain):
        if atom.model_num != "1":
...
def _get_atom_site_list(parsed_info: MmCIFDict, is_full) -> Sequence[AtomSite]:
    """Returns list of atom sites; contains data not present in the structure."""
    if parsed_info.get("_atom_site.auth_seq_id", None) is None:
        parsed_info["_atom_site.auth_seq_id"] = parsed_info["_atom_site.label_seq_id"]
...
def mmcif_loop_to_list_atom(
    ...
    Returns:
      Returns a list of dicts; each dict represents 1 entry from an mmCIF loop.
    """
    atom_ids = parsed_info["_atom_site.label_seq_id"]
    atom_ids = [list(group) for _, group in itertools.groupby(atom_ids)]
    atom_labels_temp = parsed_info["_atom_site.label_comp_id"]
    chain_ids_temp = parsed_info["_atom_site.auth_asym_id"]
    
    chain_ids = []
    atom_labels = []
    index = 0
    for ids in atom_ids:
        atom_labels.append(atom_labels_temp[index:index+len(ids)])
        chain_ids.append(chain_ids_temp[index:index+len(ids)])
        index += len(ids)
    
    atom_ids = [i[0] for i in atom_ids]
    atom_labels = [i[0] for i in atom_labels]
    chain_ids = [i[0] for i in chain_ids]
    
    
    
    return [dict(zip(["_atom_site.label_seq_id", "_atom_site.label_comp_id", "_atom_site.auth_asym_id"], [atom_id, atom_label, chain_id])) 
            for atom_id, atom_label, chain_id in zip(atom_ids, atom_labels, chain_ids)]


def _get_protein_chains_uncomplete(
    *, parsed_info: Mapping[str, Any]
) -> Mapping[ChainId, Sequence[Monomer]]:
    """Extracts polymer information for protein chains only.
    ...
    """
    # Get polymer information for each entity in the structure.
    entity_poly_seqs = mmcif_loop_to_list_atom("_atom_site.", parsed_info)

    polymers = collections.defaultdict(list)
    for entity_poly_seq in entity_poly_seqs:
        polymers[entity_poly_seq["_atom_site.auth_asym_id"]].append(
            Monomer(
                id=entity_poly_seq["_atom_site.label_comp_id"],
                num=int(entity_poly_seq["_atom_site.label_seq_id"]),
            )
        )
    ...
    for entity_id, seq_info in polymers.items():
        # Reject polymers without any peptide-like components, such as DNA/RNA.
        valid_chains[entity_id] = seq_info
