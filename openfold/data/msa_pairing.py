...
SEQ_FEATURES = ('residue_index', 'aatype', 'all_atom_positions',
                ...
                'rigid_group_default_frame', 'chaintype_id', 'region_id')
...
def create_paired_features(
  ...
  if len(chains) < 2:
    ...
    updated_chains = []
    paired_rows = pair_antibody_sequences(chains)
...
def pair_antibody_sequences(
    examples: List[Mapping[str, np.ndarray]],
) -> Dict[int, np.ndarray]:
  """Returns indices for paired antibody MSA sequences across chains."""
  
  msa_species_identifiers_dict_list = []
  species_set_list = []
  for chain_features in examples:
    id_to_seq_dict = {}
    for i, idx in enumerate(chain_features['msa_species_identifiers_all_seq']):
      id_to_seq_dict[idx] = i
    
    species_set = set(chain_features['msa_species_identifiers_all_seq'])
    species_set.remove(b'')
    msa_species_identifiers_dict_list.append(id_to_seq_dict)
    species_set_list.append(species_set)

  common_species = sorted(list(set.intersection(*species_set_list)), reverse=False, key=int)
  all_paired_msa_rows = np.array([[0]+[msa_species_identifiers_dict_list[i][sp] for sp in common_species] for i in range(len(examples))]).T
...
def _merge_features_from_multiple_chains(
  ...
  for feature_name in chains[0]:
    ...
    elif feature_name_split == 'region_index':
      merged_example[feature_name] = feats
    else:
...
def _concatenate_paired_and_unpaired_features(
  ...
  for feature_name in features:
    if feature_name in example:
      ...
      example["pair_msa_num"] = feat_all_seq.shape[0]
      example["single_msa_num"] = feat.shape[0]
...
def merge_chain_features(np_chains_list: List[Mapping[str, np.ndarray]],
  ...
  if pair_msa_sequences:
    np_example = _concatenate_paired_and_unpaired_features(np_example) # 把paired的msa和unpaired的msa拼接起来
