    ...
    if region_index is not None:
        ...
        for index in region_index:
            if index[0]["chain_type"] == "H":
                cdr3_start = index[0]["CDR3"][0] + start_index
                cdr3_end = index[0]["CDR3"][1] + start_index
                protein["template_all_atom_mask"][:, cdr3_start:cdr3_end, :] = 0 # 将CDR3区域的氨基酸序列设为0
                protein["template_all_atom_positions"][:, cdr3_start:cdr3_end, :, :] = 0
                
            if index[0]["chain_type"] == "L":
                cdr3_start = index[0]["CDR3"][0] + start_index
                cdr3_end = index[0]["CDR3"][1] + start_index
                protein["template_all_atom_mask"][:, cdr3_start:cdr3_end, :] = 0 # 将CDR3区域的氨基酸序列设为0
                protein["template_all_atom_positions"][:, cdr3_start:cdr3_end, :, :] = 0
