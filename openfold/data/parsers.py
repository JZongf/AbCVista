...
import numpy as np
from utils.blosum import cal_blusom_score, parse_blosum_matrix, blosum80_str


def cal_sequence_identity(
    seq1: str, seq2: str, insert_count, ignore_gaps: bool = False, mask=None
) -> float:
    ...
    Args:
        ...
        ignore_gaps: Whether to ignore gaps when calculating sequence identity.

    Returns:
        The sequence identity between the two sequences as a float between 0 and 1.
    """
    if ignore_gaps:
        seq1 = seq1.replace("-", "")
        seq2 = seq2.replace("-", "")
    ...
    if mask is None:
        ...
        comparison = (seq1_array == seq2_array).astype(int) * 2 - 1
        identity_score = np.sum(comparison)
        return (identity_score - insert_count) / length
    else:
        # 确保mask长度也匹配
        mask_array = np.array(mask[:length])
        seq1_array = np.array(list(seq1))
        seq2_array = np.array(list(seq2))
        
        comparison = (seq1_array == seq2_array).astype(int) * 2 - 1
        identity_score = np.sum(mask_array * comparison)
        return (identity_score - insert_count) / np.sum(mask_array)
...
def cal_sum_probs(query_sequence: str, hit_sequence: str, mask=None) -> float:
    # remove lower case letters from hit_sequence
    hit_sequence_fixed = ''.join([r for r in hit_sequence if r == '-' or r.isupper()])
    
    len_insert = len(hit_sequence) - len(hit_sequence_fixed)
    
    sum_probs = cal_sequence_identity(query_sequence, hit_sequence_fixed, len_insert, mask=mask)
...
def parse_hmmsearch_a3m(
    ...
    mask = None
    if chain_features:
        chain_type = chain_features["region_index"]["chain_type"]
        region_id = chain_features["region_id"]
        nanobody = not chain_features["multimer"]
        if chain_type == "H" and nanobody:
            mask = np.zeros_like(region_id)
            mask[(region_id == 1) | (region_id == 3)] = 1
            mask[(region_id == 2) | (region_id == 4)] = 3

    indices_query = _get_indices(query_sequence, start=0)
    parsed_info_list = [(cal_sum_probs(query_sequence, hit_sequence, mask), hit_sequence, hit_description)  for hit_sequence, hit_description in parsed_a3m]
    parsed_info_list = sorted(parsed_info_list, key=lambda x: x[0], reverse=True)
    
    hits = []
    for i, (sum_probs, hit_sequence, hit_description) in enumerate(parsed_info_list[:100], start=1):
