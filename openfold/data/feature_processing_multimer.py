...
from openfold.data import msa_pairing, sample_msa
...
REQUIRED_FEATURES = frozenset({
    ...
    'region_index', "pair_msa_num", "single_msa_num", "chain1_msa_num", "chain2_msa_num",
    "chaintype_id", "region_id",
})
...
def pair_and_merge(
  ...
  pair_msa_sequences = len(np_chains_list) > 1 
  ...
  if pair_msa_sequences:
    ...
    # # 去除未配对序列中的重复序列，但是这有个问题，单链是重复的，但是多链不一定重复
  ...
  # # 排序后剪切多序列比对到指定数量
  # np_chains_list = sample_msa.rerange_and_crop_single_msa_by_chain(np_chains_list)
  
  # # 剪切模板序列到指定数量
  # np_chains_list = crop_templates(
  #     np_chains_list,
  #     max_templates=MAX_TEMPLATES
  # )

  msa_num_list = []
  for index, np_chain in enumerate(np_chains_list):
    msa_num_list.append(len(np_chain["msa"]))
  
  np_example = msa_pairing.merge_chain_features(
      np_chains_list=np_chains_list, pair_msa_sequences=pair_msa_sequences,
      max_templates=max([np_chain["template_domain_names"].shape[0] for np_chain in np_chains_list])
  )
  np_example = process_final(np_example)
  
  np_example["msas_num"] = []
  if np_example.get("pair_msa_num", None) is not None:
    np_example["msas_num"].append(np_example["pair_msa_num"])
  
  for index, msa_num in enumerate(msa_num_list):
    np_example["msas_num"].append(msa_num)
  
  for index, msa_num in enumerate(msa_num_list):
    np_example[f"chain{index+1}_msa_num"] = msa_num
...
def crop_templates(
  ...
  """Crops the templates for a set of chains.
  ...
  for chain in chains_list:
    cropped_chain = _crop_single_template(
        chain,
        max_templates=max_templates)
...
def _crop_single_template(chain: Mapping[str, np.ndarray],
                          max_templates: int) -> Mapping[str, np.ndarray]:
  """Crops the templates for a single chain."""
...
def _crop_single_chain(chain: Mapping[str, np.ndarray],
  ...
  if pair_msa_sequences:
    ...
    msa_crop_size_all_seq = msa_size_all_seq
    ...
    msa_crop_size = np.minimum(msa_size, max_msa_crop_size//2)
    msa_crop_size = np.maximum(msa_crop_size, MIN_CROP_SIZE)
  else:
    msa_crop_size = np.minimum(msa_size, msa_crop_size)
...
def _restore_msa_restypes(msa):
  """Restore MSA restype to the original order."""
  new_order_list = residue_constants.MAP_OUR_AATYPE_TO_HHBLITS_AATYPE
  msa = np.take(new_order_list, msa, axis=0)
