...
def model_config(
    ...
    # INFERENCE PRESETS
        ...
        if re.fullmatch("^model_[1-5]_multimer(_v2)?$", name):
            ...
            c.model.template.enabled = True
            # c.loss.experimentally_resolved.weight = 0.01
            # c.loss.violation.weight = 0.03
            c.loss.tm.weight = 0.1
            c.data.train.max_msa_clusters = 64
            c.data.predict.max_msa_clusters = 64
            c.data.train.max_extra_msa = 64
            c.data.predict.max_extra_msa = 64
    else:
        ...
        "data": {
            "common": {
                "feat": {
                    ...
                    "use_clamped_fape": [],
                    "original_msa": [NUM_MSA_SEQ, NUM_RES],
                    "chaintype_id": [NUM_RES],
                    "region_id": [NUM_RES],
                },
                # "masked_msa": {
                #     "profile_prob": 0.1,
...
multimer_config_update = mlc.ConfigDict({
    ...
    "model": {
        ...
        "recycle_early_stop_tolerance": 0.5 # not finetuning
    },
    "loss": {
        "fape": {
