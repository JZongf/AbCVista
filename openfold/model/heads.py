...
class AuxiliaryHeads(nn.Module):
    ...
    def forward(self, outputs):
        ...
        if self.config.tm.enabled:
            # 将ndarray中为7的位置替换为1其他位置替换为0
            residue_weights = (outputs["region_id"] == 6).float()
            for i, chain in enumerate(outputs["chaintype_id"]):
                if chain == 2:
                    residue_weights[i] = 0
            if sum(residue_weights) == 0:
                residue_weights = None
                    
            tm_logits = self.tm(outputs["pair"])
            aux_out["tm_logits"] = tm_logits
            aux_out["ptm_score"] = compute_tm(
                tm_logits, residue_weights, **self.config.tm
            )
            asym_id = outputs.get("asym_id")
            if asym_id is not None:
                aux_out["iptm_score"] = compute_tm(
                    tm_logits, residue_weights=residue_weights, asym_id=asym_id, interface=True, **self.config.tm
                )
                aux_out["weighted_ptm_score"] = (self.config.tm["iptm_weight"] * aux_out["iptm_score"]
                                                 + self.config.tm["ptm_weight"] * aux_out["ptm_score"])
            else:
                aux_out["weighted_ptm_score"] = aux_out["ptm_score"]
